...
Dev Studio 支持通过工具 Manifest 中的 `service.type` 开发和测试三种服务：

| 类型 | 使用场景 | 测试与调用方式 |
|------|----------|----------------|
| `json` | 结构化工具/API | JSON 请求与响应，通过工具调用 API 发布 |
| `http` | Web 页面或通用 HTTP 服务 | 完整交互式 HTML 预览；原样代理请求方法、查询参数、重定向和相对静态资源 |
| `sse` | 流式 HTTP 服务 | 事件流预览，代理层关闭缓冲并透传流式响应 |

服务发布将开发环境与运行交付分离：

- 访问认证 — 可选择 API Key 或匿名访问，两种模式均提供调用文档
- 网络范围 — 默认使用内网地址 `/services/{instanceId}/`；可通过共享公网前缀或服务独立域名及反向代理对外发布
- Web 入口链接 — 已发布的 HTTP 页面会向绑定该工具的数字员工提供可点击服务链接，优先使用公网地址，否则回退内网地址
- 水平扩缩容 — 可设置或修改 1–20 个 OpenSandbox 实例（`实例-0` … `实例-n`），仅向就绪实例分发流量
- 负载均衡 — CrewMeld 服务网关使用 Redis 在应用节点间共享轮询游标，Redis 不可用时回退本地轮询，并支持浏览器会话亲和
...
- 智能客服模式 — 将数字员工限制为指定范围的知识检索，不执行 SOP 或外部工具
- 多模态输入 — 回答前可通过视觉模型或百度智能云 OCR 处理文件、图片和 PDF
- 转人工客服 — 原会话可进入人工客服工作台，完整保留消息历史
...
- QA 运营 — 管理问答条目、预览导入、CSV 导入导出，并将变更同步到 RAGFlow
- 问题分析 — 聚合真实用户问题，对相似问题分类或合并、转入 QA，并展示各品类 Top 3 高频问题
- 知识缺口追踪 — 记录无法回答或相似度不足的问题及出现次数，持续完善知识库
- 富文档交付 — 提取并绑定文档图片，使客服回答能够返回相关图文内容
...
CrewMeld 可将数字员工配置为知识驱动的智能客服，并让管理台、公共 API、独立 H5 和消息渠道复用同一套对话运行时。

- 客服角色 — 开启智能客服模式，并配置支持 `{{user.name}}`、`{{user.email}}` 等身份变量的开场白
- 分类咨询 — 用户可选择产品或业务知识库，后续检索和推荐问题均限定在当前品类
- 高频问题 — 根据真实咨询历史，为每个品类展示最多三个高频问题
- 多模态理解 — 使用视觉模型或百度 OCR 处理上传文件和安全 HTTPS 附件，并可设置来源域名白名单
- 知识运营 — 维护文档与 QA 知识库、分析重复问题，并查看 AI 无法可靠回答的问题
- 人工接待 — 转人工会话分为待接待、处理中和已结束；授权客服可以认领、回复并结束会话
- 独立客服 H5 — 支持创建会话、查看历史、上传文件、选择知识品类及接收 SSE 流式回复
- 数字员工 API — 将完整对话能力对外发布，支持按 Key 设置身份用户、来源域名、客服人员白名单，并提供 OpenAPI 与 Swagger 文档
...
└─────────────────────────────────────────────────────────────┘
                          ↓ LLM 决策调用工具
┌─────────────────────────────────────────────────────────────┐
│  工具与服务层 │ OpenSandbox 隔离 Node.js / Python 进程       │
│               │ JSON 工具与 HTTP/SSE 服务通过用户添加的      │
│               │「连接」操作外部系统                          │
└─────────────────────────────────────────────────────────────┘
                          ↓ 通过用户添加的连接访问
┌─────────────────────────────────────────────────────────────┐
...
```text
Web / 客服 H5 / 消息渠道 / 数字员工 API
                    │
                    ▼
        身份解析 + API Key + Origin 校验
                    │
                    ▼
             统一 Conversation Runtime
          ┌─────────┼───────────┐
          ▼         ▼           ▼
   范围化 RAG/QA  OCR 与附件   人工客服转接
          │         │           │
          ▼         ▼           ▼
   RAGFlow + 数据库 视觉/百度 OCR 人工客服工作台
          └─────────┼───────────┘
                    ▼
          SSE 回复 + 会话历史持久化
                    │
...
所有入口共用同一个对话运行时。智能客服模式通过运行时约束只允许有知识依据的检索回答，并跳过 SOP 和工具执行。PostgreSQL 作为 QA 条目、问题分析与人工转接状态的管理真源，RAGFlow 负责文档解析与检索。
...
CrewMeld 在应用层服务网关中完成服务实例负载均衡；只有绑定公网域名时才需要 Nginx 或 Ingress 等反向代理。扩缩容会协调独立的 OpenSandbox 运行实例，仅向就绪实例路由，并完整保留 HTTP 路径、查询参数、响应体、重定向和 SSE 事件流。
...
| 工具与服务运行时 | OpenSandbox（Docker / Kubernetes 运行时） |
| 服务网关 | Next.js HTTP/SSE 透明代理 + Redis 轮询负载均衡 |
| 智能客服 | 统一对话运行时 + H5 客户端 + 人工客服工作台 |
| OCR | 多模态大模型或百度智能云 OCR |
| 认证 | better-auth |
...
│   ├── providers/            # LLM 适配器
│   ├── tools/                # HTTP 工具实现
│   └── lib/                  # SOP/对话/智能客服/服务网关/渠道/K8s/i18n
├── apps/customer-service-h5/ # 独立智能客服 Web 客户端
├── packages/
│   ├── db/                   # 数据库 Schema + 迁移
│   └── logger/               # 结构化日志
...
| 变量 | 说明 |
|------|------|
| `CREWMELD_SERVICE_PUBLIC_BASE_URL` | 已发布服务的共享公网前缀，例如 `https://services.example.com`；需将 `/services/{instanceId}/` 代理到 CrewMeld |
