...
import * as schema from '@crewmeld/db/schema'
...
import { getPlatformSetting, getRegistrationSettings } from '@/lib/auth/platform-settings'
...
export const auth = betterAuth({
    ...
    user: {
      create: {
        after: async (user) => {
          ...
          try {
            const userCount = await db.select({ id: schema.user.id }).from(schema.user).limit(2)
            if (userCount.length === 1) {
              await db
                .update(schema.user)
                .set({ isSuperUser: true })
                .where(eq(schema.user.id, user.id))
              logger.info('[databaseHooks.user.create.after] First user promoted to super_admin', {
                userId: user.id,
              })
            } else {
              const approvalRequired = await getPlatformSetting('registration_approval_required')
              if (approvalRequired) {
                await db
                  .update(schema.user)
                  .set({ approvalStatus: 'pending' })
                  .where(eq(schema.user.id, user.id))
                logger.info('[databaseHooks.user.create.after] User marked as pending approval', {
                  userId: user.id,
                })
              }
            }
          } catch (error) {
            logger.error('[databaseHooks.user.create.after] Failed to check first-user promotion', {
              userId: user.id,
              error,
            })
          }
  ...
  hooks: {
    before: createAuthMiddleware(async (ctx) => {
      ...
      if (ctx.path.startsWith('/sign-up') && regSettings.registrationDisabled) {
        throw new Error(t('authRegistrationClosed'))
      }

      if (
        (ctx.path.startsWith('/sign-in') || ctx.path.startsWith('/sign-up')) &&
        (regSettings.allowedEmails || regSettings.allowedDomains)
      ) {
        ...
        if (requestEmail) {
          ...
          if (regSettings.allowedEmails) {
            const allowedEmails = regSettings.allowedEmails
              .split(',')
          ...
          if (!isAllowed && regSettings.allowedDomains) {
            const allowedDomains = regSettings.allowedDomains
              .split(',')
              .map((domain: string) => domain.trim().toLowerCase())
              .filter(Boolean)
            const emailDomain = requestEmail.split('@')[1]
            isAllowed = Boolean(emailDomain && allowedDomains.includes(emailDomain))
          }
      ...
      if (ctx.path.startsWith('/sign-in/email')) {
        ...
        if (email) {
          ...
          if (targetUser?.approvalStatus === 'pending') {
            throw new Error(t('authPendingApproval'))
          }
          if (targetUser?.approvalStatus === 'rejected') {
            throw new Error(t('authApprovalRejected'))
          }
        }
      }
