...
export const env = createEnv({
  ...
  server: {
    ...
    FROM_EMAIL_ADDRESS:                    z.string().min(1).optional(),           // Complete from address (e.g., "CrewMeld <noreply@domain.com>" or "noreply@domain.com")
    ...
    K8S_API_TOKEN:                         z.string().optional(),                  // Kubernetes bearer token
    K8S_DEPLOY_NAMESPACE:                  z.string().optional().default('crewmeld-skills'), // Namespace for skill Pods
    K8S_NODE_IP:                           z.string().optional(),                  // Node IP for NodePort access
    K8S_SKIP_TLS_VERIFY:                   z.boolean().optional(),                 // Skip TLS verification for K8s API (dev only)
    K8S_IMAGE_NODE:                        z.string().optional(),                  // Docker image for Node.js skill Pods
    K8S_WARM_POOL_SIZE:                    z.string().optional(),                  // Warm pool size (number of pre-created Pods)
    K8S_WARM_POOL_IMAGE:                   z.string().optional(),                  // Docker image for warm pool Pods

    // Connector Encryption
    CONNECTOR_ENCRYPTION_KEY:              z.string().optional(),                  // AES key for encrypting system connection credentials
  ...
  client: {
    ...
    NEXT_PUBLIC_APP_URL:                   z.string().url(),                       // Base URL of the application (e.g., https://www.crewmeld.com)
    ...
    NEXT_PUBLIC_BRAND_NAME:                z.string().optional(),                  // Custom brand name 
